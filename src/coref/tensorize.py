...
class CorefDataProcessor:
    ...
    def get_tensor_examples(self):
        ...
        if os.path.exists(cache_path):
            ...
            tensorizer = Tensorizer(self.config, self.tokenizer)
            try:
                metric_sg_suffix = f".{self.config['metric']}_{self.config['percentage']}"
                metric_file_dir = "_metric"
            except:
                metric_sg_suffix = ""
                metric_file_dir = ""
            paths = {
            ...
            singleton_paths = {
                ...
                'dev': join(self.data_dir, self.config['dataset']+'_'+self.config["singleton_suffix"]+metric_file_dir, f'dev_{self.config["singleton_suffix"]}{metric_sg_suffix}.{self.language}.{self.max_seg_len}.jsonlines'),
                'tst': join(self.data_dir, self.config['dataset']+'_'+self.config["singleton_suffix"]+metric_file_dir, f'test_{self.config["singleton_suffix"]}{metric_sg_suffix}.{self.language}.{self.max_seg_len}.jsonlines')
            }
            for split, path in paths.items():
                logger.info('Tensorizing examples from %s; results will be cached)' % path)
                logger.info('Tensorizing examples from %s; results will be cached)' % singleton_paths[split])
